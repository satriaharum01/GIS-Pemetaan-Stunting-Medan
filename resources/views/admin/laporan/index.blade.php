...
<script>
    ...
    $(function() {
        ...
        table = $('#data-width').DataTable({
            ...
            ajax: {
                url: '{{url("laporan/stunting/json/0")}}'
            },
            columns: [{
                    data: 'DT_RowIndex',
                    name: 'DT_RowIndex',
    ...
    $("body").on("click", ".btn-edit", function() {
        ...
        $('#upt').children().remove().end();
        $('#upt').append('<option value="0" disabled selected>-- Pilih UPT --</option>');
        var id = jQuery(this).attr("data-id");
        $.ajax({
            url: "{{ url('/data/upt/select')}}/" + id,
            type: "GET",
            cache: false,
            dataType: 'json',
            success: function(dataResult) {
                ...
                var resultData = dataResult.data;
                $.each(resultData, function(index, row) {
                    $('#upt').append('<option value="' + row.id_upt + '">' + row.id_upt + ' - ' + row.nama_upt + '</option>');
                })
            }
        });
    ...
    function changetahun(val) {
        ...
        table.ajax.url('{{url("laporan/stunting/json")}}/' + val).load();
        if (val === '0') {
            if (stat === 1) {
